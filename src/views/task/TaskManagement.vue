...
<template>
  <my-page-header-wrapper>
              ...
              <a-row type="flex" :gutter="16">
                <a-col>
                  <drawer-view title="新增部署任务">
                    <template slot="click">
                      <a-button type="primary"> 新增部署任务<a-icon type="plus" /> </a-button>
                    </template>
    ...
    <a-card :bordered="false" class="aops-theme" style="margin-top: 20px">
      ...
      <div style="text-align: center" v-if="templateData.length > showIndex">
  ...
  methods: {
    ...
    deleteTaskBash(rowKeys, rows) {
      ...
      this.$confirm({
        title: (
          <div>
            <p>{`确认批量删除以下${rowKeys.length}个任务：`}</p>
          </div>
        ),
    ...
    executeTask(record) {
      ...
      this.$confirm({
        title: (
          <div>
            <p>{`确认执行任务：${record.task_name}？`}</p>
          </div>
        ),
        content: (
          <div>
            <p>{hostNameList ? `该任务可能会修改以下主机配置：${hostNameList.join('、')}` : ''}</p>
            <p>详情请查看任务描述。</p>
          </div>
        ),
        icon: () => <a-icon type="exclamation-circle" />,
    ...
    executeTaskBash(rowKeys, rows) {
      ...
      this.$confirm({
        title: (
          <div>
            <p>{`确认批量执行以下${rowKeys.length}个任务：`}</p>
          </div>
        ),
        content: () => <div class="aops-app-list-in-modal">{list}</div>,
        icon: () => <a-icon type="exclamation-circle" />,
