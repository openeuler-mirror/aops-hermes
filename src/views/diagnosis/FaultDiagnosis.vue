<template>
  <my-page-header-wrapper>
    ...
    <a-card :bordered="false" class="aops-theme" style="margin-top: 20px">
          ...
          <a-list-item slot="renderItem" slot-scope="item">
            <template v-if="!item.tree_name">
              <drawer-view title="新增故障树">
                <template slot="click">
                  <a-button class="new-btn" type="dashed"> <a-icon type="plus" />新增故障树 </a-button>
                </template>
                <template slot="drawerView">
      ...
      <div style="text-align: center" v-if="treeData.length > showIndex">
...
const reportListColumns = [
  ...
  },
  {
    ...
    title: '诊断时间段',
    customRender: (text, item) =>
      `${dateFormat('YYYY-mm-dd HH:MM:SS', item.start * 1000)} - ${dateFormat('YYYY-mm-dd HH:MM:SS', item.end * 1000)}`
  }
];
