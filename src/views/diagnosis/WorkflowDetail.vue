    ...
    <div class="diagnosis-workflow-detail">
      <a-card :bordered="false" class="aops-theme detail-card">
        ...
        <a-row :gutter="8">
          ...
          <a-col :span="6"> 应用：{{ workflow.app_name }} </a-col>
          <a-col :span="6">
            状态：<span :style="activation()">{{ statusMap[workflow.status] }}</span>
          </a-col>
        </a-row>
        <a-row :gutter="8">
          <a-col :span="18" class="workflow-desc"> 描述：{{ workflow.description }} </a-col>
    ...
    buildMulticheckData() {
      ...
      for (const hostId in this.workflow.detail.multicheck) {
        ...
        const temp = Object.assign(
          temp1,
          this.workflow.input.hosts[hostId],
          this.workflow.model_info[this.workflow.detail.multicheck[hostId]]
        );
        tempArr.push(temp);
      }
